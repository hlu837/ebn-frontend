...
class ChatService {
  ...
  /// POST /api/chat/threads — gets or creates the one general (not tied
  /// to any listing) thread with this broker. Used by the "Chat" button
  /// on a broker's profile when they have no active listings to open a
  /// per-listing thread against.
  Future<ChatThread> openThreadForBroker({required String token, required String brokerId}) async {
    final res = await _post('/api/chat/threads', {'brokerId': brokerId}, token: token);
    return ChatThread.fromJson(res);
  }
