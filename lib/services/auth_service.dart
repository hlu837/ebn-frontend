...
class AuthService {
  ...
  /// POST /api/auth/forgot-password — requests a password reset code by
  /// email. The backend always returns the same generic message whether
  /// or not the email is registered, so there's nothing to branch on here
  /// beyond network/server errors.
  Future<String> forgotPassword(String email) async {
    final res = await _post('/api/auth/forgot-password', {'email': email.trim()});
    return res['message'] as String? ??
        "If an account exists for that email, we've sent a password reset code.";
  }

  /// POST /api/auth/reset-password — completes the reset using the code
  /// emailed by [forgotPassword].
