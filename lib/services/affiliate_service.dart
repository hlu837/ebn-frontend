...
class AffiliateMembershipData {
  ...
  // Admin-configurable price for every tier (from the membership_pricing
  // table), so the upgrade picker can show live prices instead of a
  // hardcoded client-side map that goes stale whenever the admin changes
  // pricing.
  ...
  factory AffiliateMembershipData.fromJson(Map<String, dynamic> json) => AffiliateMembershipData(
        ...
        billingHistory: (json['billingHistory'] as List<dynamic>? ?? [])
            ...
            .toList(),
        tierPricing: (json['tierPricing'] as Map<String, dynamic>? ?? {})
...
/// One row of the Expert's payout ledger — GET
/// /api/affiliates/me/expert-wallet. Mirrors
/// specialist_wallet_transactions (085_specialist_wallet_transactions.sql):
/// 'credit' rows are positive (escrow released to them), 'payout' rows are
/// negative (admin recorded an offline payment already made).
class ExpertWalletTransaction {
  final String id;
  ...
  factory ExpertWalletTransaction.fromJson(Map<String, dynamic> json) {
    return ExpertWalletTransaction(
      id: json['id'] as String,
...
/// GET /api/affiliates/me/expert-wallet response.
class ExpertWalletSummary {
  final int balanceCents;
  final List<ExpertWalletTransaction> transactions;

  const ExpertWalletSummary({required this.balanceCents, required this.transactions});

  double get balanceBirr => balanceCents / 100;

  factory ExpertWalletSummary.fromJson(Map<String, dynamic> json) {
    return ExpertWalletSummary(
      balanceCents: (json['balanceCents'] as num?)?.toInt() ?? 0,
      transactions: (json['transactions'] as List<dynamic>? ?? [])
          .map((e) => ExpertWalletTransaction.fromJson(e as Map<String, dynamic>))
          .toList(),
    );
  }
}
...
class AffiliateService {
  ...
  // ── Expert profile (Expert merged into the Affiliate dashboard) ─────────

  /// GET /api/affiliates/me/expert-profile — null if this Affiliater
  /// hasn't set one up yet.
  Future<ServiceProvider?> getExpertProfile(String token) async {
    final json = await _get('/api/affiliates/me/expert-profile', token: token);
    final profile = json['profile'] as Map<String, dynamic>?;
    return profile == null ? null : ServiceProvider.fromJson(profile);
  }

  /// POST /api/affiliates/me/expert-profile — becomes an Expert. Starts
  /// inactive until an admin approves it in Admin Settings.
  Future<ServiceProvider> createExpertProfile(
    String token, {
    ...
    final json = await _post('/api/affiliates/me/expert-profile', {
      'name': name,
      'category': category,
      'phone': phone,
      'city': city,
    ...
    return ServiceProvider.fromJson(json['profile'] as Map<String, dynamic>);
  }

  /// PUT /api/affiliates/me/expert-profile — edit an existing Expert profile.
  Future<ServiceProvider> updateExpertProfile(
    String token, {
    String? name,
    String? category,
    String? phone,
    ...
    final json = await _put('/api/affiliates/me/expert-profile', {
  ...
  /// GET /api/affiliates/me/expert-jobs — maintenance jobs assigned to
  /// this Expert. Returned as raw JSON so callers can feed it straight to
  /// [MaintenanceAssignment.fromJson] without a circular import here.
  Future<List<Map<String, dynamic>>> getExpertJobs(String token) async {
    final res = await _getRaw('/api/affiliates/me/expert-jobs', token: token);
    final list = jsonDecode(res.body) as List<dynamic>;
    return list.cast<Map<String, dynamic>>();
  }

  /// GET /api/affiliates/me/expert-wallet — balance + payout ledger.
  Future<ExpertWalletSummary> getExpertWallet(String token) async {
    final json = await _get('/api/affiliates/me/expert-wallet', token: token);
    return ExpertWalletSummary.fromJson(json);
  }
  ...
  Future<Map<String, dynamic>> _put(String path, Map<String, dynamic> body, {required String token}) async {
    http.Response res;
    try {
      res = await http.put(
        _uri(path),
