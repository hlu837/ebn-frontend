...
class AssetService {
  ...
  Future<Asset> updateAsset(
    ...
    final json = await _patch(
      ...
      token: token,
      // Listing photos are embedded as base64 data URLs, so an edit that
      // touches images can be a multi-megabyte upload — the default 15s
      // timeout was tripping on slower mobile connections and surfacing
      // as "editing doesn't work". Give image-carrying saves more room.
      timeout: (imageUrls != null && imageUrls.isNotEmpty)
  ...
  /// Requires an admin [token]: the route is behind requireAuth +
  /// requireAdmin server-side, so an omitted/blank token is a 401.
  Future<void> deleteAsset(String id, {String? token}) async {
    http.Response res;
    try {
      res = await http
          .delete(_uri('/api/assets/$id'), headers: {
