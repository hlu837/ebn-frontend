...
class AdminService {
  ...
  /// GET /api/users?accountStatus=pending_approval — every account
  /// waiting on an admin decision. Property Owner is currently the only
  /// signup that lands in this state (see 064_property_owner_role.sql),
  /// so this doubles as "pending Property Owners" today; filtered
  /// defensively by [AdminUser.isPendingPropertyOwner] on the caller
  /// side in case another pending-approval role is added later.
  Future<List<AdminUser>> fetchPendingPropertyOwners({required String token}) async {
    final json = await _get(
        '/api/users?accountStatus=pending_approval&limit=200', token: token);
    final rows = (json['users'] as List<dynamic>).cast<Map<String, dynamic>>();
    return rows.map(AdminUser.fromJson).toList();
  }

  /// PATCH /api/users/:id/approve-pending-role — activates a pending
  /// signup: role becomes its pendingRole, accountStatus becomes 'active'.
  Future<AdminUser> approvePendingRole(String userId, {required String token}) async {
    final json = await _patch('/api/users/$userId/approve-pending-role', const {}, token: token);
    return AdminUser.fromJson(json);
  }

  /// PATCH /api/users/:id/reject-pending-role — declines a pending
  /// signup. The account isn't deleted; it's demoted back to a plain
  /// active Visitor (role='user') since the email/password are already real.
  Future<AdminUser> rejectPendingRole(String userId, {required String token}) async {
    final json = await _patch('/api/users/$userId/reject-pending-role', const {}, token: token);
    return AdminUser.fromJson(json);
  }
