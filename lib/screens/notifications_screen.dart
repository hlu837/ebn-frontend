...
class NotificationsScreen extends StatefulWidget {
  const NotificationsScreen({super.key, required this.token, this.onUnreadCountChanged, this.user});

  final String token;
  final ValueChanged<int>? onUnreadCountChanged;

  /// The signed-in user. Needed only to deep-link a rental-agreement
  /// notification into [MyRentalAgreementsScreen]; when null, tapping one
  /// just behaves like any other notification (pops back with it).
  final AppUser? user;
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  IconData _iconFor(AppNotificationKind kind) {
    switch (kind) {
      ...
      case AppNotificationKind.rentalAgreement:
        ...
        itemBuilder: (context, i) {
          ...
          return InkWell(
            ...
            onTap: () {
              ...
              if (n.kind == AppNotificationKind.rentalAgreement && user != null) {
                // Land on the agreement itself (its terms / payment step),
                // not back on the feed. push (not replace) so Back returns
                // to the notification list.
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (_) => MyRentalAgreementsScreen(user: user, openAgreementId: n.relatedId),
                  ),
                );
                return;
              }
