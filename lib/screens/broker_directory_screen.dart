...
class _BrokerDirectoryScreenState extends State<BrokerDirectoryScreen> {
  ...
  /// [_brokers] narrowed down by [_query] — matched against name, company,
  /// and city so searching either "Sarah", her agency, or her city all
  /// find the same broker. Empty query (the common case) returns the
  /// full list untouched.
  ...
  void _openProfile(Broker broker) {
    Navigator.of(context).push(MaterialPageRoute(
      builder: (_) =>
          BrokerProfileScreen(broker: broker, currentUser: widget.currentUser),
    ));
  }
  ...
  Widget build(BuildContext context) {
    final brokers = _filteredBrokers;
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: _BrokerSearchField(controller: _searchController),
            ),
            Expanded(
              child: _loading
                  ? const Center(child: CircularProgressIndicator())
                  : _error != null
                      ? _DirectoryErrorState(
                          message: _error!, onRetry: _loadBrokers)
                      : _tab == 0
                          ? (brokers.isEmpty && _query.trim().isNotEmpty
                              ? _NoSearchResultsState(query: _query)
                              : _BrokerListView(
                                  brokers: brokers,
...
/// Search-by name/company/city bar for the broker list — lives just under
/// the header, above both the List and Map tabs, so switching tabs keeps
/// whatever's typed and narrows either view the same way.
class _BrokerSearchField extends StatelessWidget {
  const _BrokerSearchField({required this.controller});
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: 'Search agents by name, agency, or city...',
        hintStyle: const TextStyle(color: LandingColors.muted, fontSize: 13.5),
        prefixIcon: const Icon(Icons.search_rounded,
            color: LandingColors.muted, size: 20),
          ...
          builder: (context, value, _) {
            ...
            return IconButton(
              icon: const Icon(Icons.close_rounded,
                  color: LandingColors.muted, size: 18),
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.search_off_rounded,
                size: 36, color: LandingColors.muted),
            const SizedBox(height: 12),
            Text(
              'No agents match "$query".',
              textAlign: TextAlign.center,
              style:
                  const TextStyle(color: LandingColors.muted, fontSize: 13.5),
            ),
          ],
        ),
      ),
          ...
          children: [
            ...
            Text(message,
                ...
                style: const TextStyle(
                    color: LandingColors.muted, fontSize: 13.5)),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
              ...
              InkWell(
                ...
                child: const Padding(
                  ...
                  child: Icon(Icons.arrow_back,
                      size: 22, color: LandingColors.foreground),
                ),
              ),
              Expanded(
                child: Text(
                  'Verified agents who can help you',
                  style: const TextStyle(
                    fontSize: 22,
          ...
          Text(
            '$categoryLabel brokers ready to close this deal.',
            style: const TextStyle(
                fontSize: 14.5, color: LandingColors.muted, height: 1.35),
          ),
        ],
      ),
    );
          ...
          child: Row(
            ...
            children: [
              Icon(icon,
                  size: 15,
                  color:
                      selected ? LandingColors.primaryFg : LandingColors.muted),
...
class _BrokerRow extends StatelessWidget {
  const _BrokerRow(
      {required this.broker, required this.onTap, required this.onMapTap});
  ...
  @override
  Widget build(BuildContext context) {
    final avatarImage = brokerAvatarImage(broker);
    return InkWell(
      onTap: onTap,
      child: Padding(
                  ...
                  Row(
                    children: [
                      ...
                      Icon(broker.tier.icon,
                          size: 15, color: broker.tier.color),
                      const SizedBox(width: 3),
                      Text(
                        broker.tier.label,
                        style: TextStyle(
            ...
            InkWell(
              ...
              child: const Padding(
                ...
                child: Icon(Icons.map_outlined,
                    size: 20, color: LandingColors.foreground),
              ),
            ),
            const SizedBox(width: 2),
            const Icon(Icons.chevron_right_rounded,
                size: 22, color: LandingColors.muted),
          ],
        ),
      ),
    );
            ...
            child: widget.highlighted == null
                ? Container(
                    ...
                    child: const Text(
                      ...
                      textAlign: TextAlign.center,
                      style:
                          TextStyle(color: LandingColors.muted, fontSize: 12.5),
                    ),
                  )
