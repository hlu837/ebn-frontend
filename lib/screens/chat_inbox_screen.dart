...
class _ChatInboxScreenState extends State<ChatInboxScreen> {
  ...
  String _nameFor(api.ChatThread t) => t.otherParty?.fullName.trim().isNotEmpty == true ? t.otherParty!.fullName : 'Customer';

  List<api.ChatThread> get _visibleThreads {
    final query = _search.text.trim().toLowerCase();
    if (query.isEmpty) return _threads;
    return _threads.where((t) => _nameFor(t).toLowerCase().contains(query)).toList();
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: _searching
            ...
            : const Text('Messages', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w800, color: AppColors.ink)),
        actions: [
          IconButton(
    ...
    return const Center(
      child: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.search_off_rounded, size: 40, color: AppColors.slate),
            SizedBox(height: 12),
            Text('No conversations match that name', style: TextStyle(fontSize: 14, color: AppColors.slate)),
          ],
        ),
      ),
    );
