...
class _AgentSellRequestsScreenState extends State<AgentSellRequestsScreen>
  ...
  Future<void> _refreshAgentView() async {
    if (!mounted) return;
    await context.read<SellRequestController>().fetchAgentView(widget.user.id);
  }
      ...
      body: TabBarView(
        ...
        children: [
          (nearby.isEmpty && open.isEmpty)
              ? const _EmptyState(
                  message: 'No properties waiting for a broker right now.')
              : ListView(
          ...
          mine.isEmpty
              ? const _EmptyState(
                  message: "You haven't claimed any properties yet.")
              : ListView.separated(
                  padding: const EdgeInsets.all(AppSpacing.lg),
                    ...
                    return _RequestCard(
                      ...
                      onAction: canSubmit
                          ? () => Navigator.of(context).push(MaterialPageRoute(
                                builder: (_) =>
                                    AgentPropertyReportScreen(request: r),
                              ))
                          : null,
                    );
                  },
