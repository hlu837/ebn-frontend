...
class _AgentBrokerNetworkScreenState extends State<AgentBrokerNetworkScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    try {
      specialties = List<AssetCategorySlug>.of(_brokers
          .expand((b) => b.specialties)
          // "Shop" (the `others` slug) is deliberately excluded from the
          // horizontal filter strip — brokers can still carry it as a
          // specialty, it just isn't offered as a filter here.
          .where((s) => s != AssetCategorySlug.others)
          .toSet())
...
class _MapPreviewCard extends StatelessWidget {
  const _MapPreviewCard(
      {required this.broker, required this.distanceKm, required this.currentUser});
  final Broker broker;
    ...
    return Material(
      ...
      child: InkWell(
        ...
        onTap: () => showModalBottomSheet<void>(
          ...
          builder: (_) => _BrokerDetailSheet(
              broker: b, distanceKm: distanceKm, currentUser: currentUser),
        ),
        child: Container(
  ...
  void _openDetail(BuildContext context) {
    showModalBottomSheet<void>(
      ...
      shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
      builder: (_) => _BrokerDetailSheet(
          broker: broker, distanceKm: distanceKm, currentUser: currentUser),
    );
  }
...
class _BrokerDetailSheet extends StatefulWidget {
  const _BrokerDetailSheet(
      {required this.broker, this.distanceKm, required this.currentUser});
  final Broker broker;
  final double? distanceKm;
  final AppUser currentUser;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: AppSpacing.md),
            // The "active listings" stat chip above used to be dead text —
            // no way to actually see the selected broker's properties from
            // here. This opens their real listing list (read-only), same
            // screen/filters an agent gets for their own listings.
            SizedBox(
              width: double.infinity,
              child: OutlinedButton.icon(
                onPressed: () {
                  Navigator.of(context).push(MaterialPageRoute(
                    builder: (_) => AgentListingsScreen(
                      user: widget.currentUser,
                      viewedBrokerId: b.id,
                      viewedBrokerName: b.name,
                    ),
                  ));
                },
                icon: const Icon(Icons.home_work_outlined, size: 18),
                label: Text(listingsCount == null
                    ? 'View properties'
                    : 'View properties ($listingsCount active)'),
              ),
            ),
            const SizedBox(height: AppSpacing.sm),
            Row(
