...
class _AgentVisibilityProfileScreenState
  ...
  /// A specific area/neighborhood within [_city] (e.g. "Jemo") — shown on
  /// the public profile and map pin so customers can tell brokers in the
  /// same city apart, instead of everyone just showing "Addis Ababa".
  ...
  ImageProvider<Object>? _cachedAvatarImage;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        ...
        _cachedAvatarImage = null; // Clear cache when URL changes
        _specialties
          ..clear()
  ...
  ImageProvider<Object>? get _avatarImage {
    ...
    if (avatarUrl.startsWith('data:')) {
      _cachedAvatarImage = MemoryImage(base64Decode(avatarUrl.split(',').last));
    } else {
      _cachedAvatarImage = NetworkImage(avatarUrl);
    }
          ...
          : _loadError != null
              ...
              : ListView(
                  ...
                  children: [
                    ...
                    const Text(
                        'The neighborhood or sub-city you work in — shown to customers alongside your city so they can find someone nearby.',
                        style:
                            TextStyle(fontSize: 11.5, color: AppColors.slate)),
                    const SizedBox(height: 6),
                    TextField(
                        controller: _addressLine,
                        onChanged: (_) => _markDirty(),
                        decoration: const InputDecoration(
                            hintText: 'e.g. Jemo, Bole, Kazanchis')),
                    const SizedBox(height: AppSpacing.md),
                    ...
                    Wrap(
                      ...
                      children: AssetCategorySlug.values.map((c) {
                        ...
                        // Once the tier's cap is hit, any *unselected* chip is
                        // effectively locked — grey it out so it's visually
                        // obvious tapping it won't do anything silently, and
                        // still surface the SnackBar explanation on tap.
                            ...
                            if (v) {
                              if (_maxSpecialties < 999 &&
                                ...
                                final message = widget.tier == AgentTier.bronze
                                    ? 'Bronze members can only select 1 property type. Upgrade to Silver or Gold to pick more.'
                                    : 'Silver members can select up to 3 property types. Upgrade to Gold for unlimited access.';
                                ScaffoldMessenger.of(context)
                                  ..hideCurrentSnackBar()
                                    ...
                                    action: SnackBarAction(
                                      ...
                                      onPressed: () {
                                        Navigator.of(context).push(
                                          MaterialPageRoute(
                                            builder: (_) =>
                                                AgentMembershipScreen(
                                                    user: widget.user,
                                                    initialTier: widget.tier),
                                          ),
                                        );
                                      },
                                    ),
                          ...
                          labelStyle: TextStyle(
                              ...
                              color: selected
                                  ...
                                  : atLimit
                                      ...
                                      : AppColors.ink),
                          backgroundColor:
                              atLimit ? AppColors.card.withOpacity(0.5) : AppColors.card,
