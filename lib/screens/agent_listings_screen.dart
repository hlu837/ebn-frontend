...
/// Every property this Broker/Agent has posted — any status — with a
/// status filter strip so "Active" (what a customer can currently see
/// live) is easy to isolate from drafts, sold, or archived listings.
///
/// This is the agent-side counterpart to `PropertyOwnerListingsScreen`;
/// before this screen existed, the only place a broker could see their
/// own listings was a 2-item preview on the dashboard whose "See All"
/// button pointed at the sell-requests workflow instead of their actual
/// posted properties, so most of a broker's own list was never visible
/// and there was no way to filter it down to just the active ones.
class AgentListingsScreen extends StatefulWidget {
  const AgentListingsScreen({super.key, required this.user});
...
class _ListingCard extends StatelessWidget {
  const _ListingCard(
      {required this.asset, required this.user, required this.onChanged});
