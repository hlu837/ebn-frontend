      ...
      child: ListView(
        ...
        children: [
          ...
          Builder(builder: (context) {
            // Same numbers the Property Management screen's tabs show, so
            // the dashboard and that screen never disagree.
            final sellRequests = context.watch<SellRequestController>();
            final availableCount =
                sellRequests.broadcastingFor(user.id).length +
                    sellRequests.openToBrokers.length;
            // Claimed orders count here too — they're listed under the same
            // screen's "My claims" tab, so both must agree.
            final claimedCount = sellRequests.claimedBy(user.id).length +
                sellRequests.reportsPendingBy(user.id).length +
                context.watch<OrderRequestController>().claimedActiveBy(user.id).length;
            return Column(
                  ...
                  children: [
                    ...
                    TextButton(
                      ...
                      onPressed: () => Navigator.of(context).push(
                          MaterialPageRoute(
                              builder: (_) =>
                                  AgentSellRequestsScreen(user: user))),
            ...
            Align(
              ...
              child: TextButton(
                ...
                onPressed: () => Navigator.of(context).push(MaterialPageRoute(
                    builder: (_) => AgentListingsScreen(user: user))),
                child: const Text('View all my listings',
                    style: TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.w800,
                        color: AppColors.primaryYellow)),
...
class _Header extends StatelessWidget {
  ...
  // Uses the cached helper (not an inline base64Decode) so the same
  // ImageProvider instance is reused across rebuilds — this screen's
  // header rebuilds every second while a countdown/poll timer is running,
  // and a freshly-decoded MemoryImage each time made the avatar visibly
  // flicker/reload.
  ImageProvider<Object>? get _avatarImage => dataUrlOrNetworkImage(avatarUrl);
...
/// Compact "Available (n) / Claimed (n)" pill used under the dashboard's
/// Property Management header. Tapping either one opens the full Property
/// Management screen, where the same numbers appear as tab labels.
