...
class _AgentHomeScreenState extends State<AgentHomeScreen> {
  ...
  // Track pending order alert overlay
  OrderRequest? _pendingOrderAlert;
  Timer? _orderAlertCountdown;
  int _orderAlertSecondsLeft = 30;
  ...
  // Tour requests we've already surfaced the full-screen claim alert for.
  // The backend can emit a *fresh* notification (new notification id) each
  // time it re-broadcasts/re-dispatches the same underlying tour request to
  // this agent — without this, every one of those re-notifications popped
  // the alert again, which is what made it feel like it kept firing on
  // every other screen. Keyed by the OrderRequest id, not the notification
  // id, so the same tour request only ever interrupts the agent once.
  final Set<String> _alertedOrderRequestIds = {};
  ...
  // Backing value for the dashboard's quick-access language icon. Kept in
  // sync with `GET/PATCH /api/agents/:id/settings` — the same endpoint the
  // full Settings screen uses — so switching language from either place
  ...
  void initState() {
    ...
    // Reflect the account's actual saved state, not a hard-coded "off" —
    // `_hasLocation` mirrors whether this agent already has a location on
    // file (from `widget.user`, loaded fresh at login). Without this, the
    // switch renders off on every app open/restart even for an agent who's
    // still online server-side, which reads as "the toggle won't save."
    _isOnline = _hasLocation;
    _loop!.setOnlineStatus(_isOnline);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ...
        // Populates the claim pool + this agent's claims so the dashboard's
        // Property Management counters are correct on first load, instead
        // of only after the agent opens the Property Management screen
        // (which is what used to trigger the only fetchAgentView call).
        context.read<SellRequestController>().fetchAgentView(widget.user.id);
      }
    });
    _loadAssets();
    _loadMyProperties();
  ...
  Future<void> _alertOnNewNotifications(
    ...
    final firstFresh = fresh.first;
    // Auto-pop the full-screen claim alert for a genuinely new dispatch —
    // but only ONCE per underlying request. The backend can emit a fresh
    // notification (new notification id) each time it re-broadcasts the
    // *same* tour/order request to this agent (e.g. after another agent's
    // response window expires), and without the request-id guard below
    // that re-triggered this same full-screen popup over and over, which
    // is the "fake alert every couple seconds" bug.
    if (shouldAutoOpenDispatchAlert(firstFresh) && _pendingOrderAlert == null) {
      final request =
          await OrderRequestService().getById(firstFresh.relatedId!);
      if (!mounted || request == null) {
        NotificationAlertOverlay.show(
          context,
          notification: firstFresh,
          onTap: () => _handleNotificationTap(firstFresh),
        );
        return;
      }
      if (!_pendingOrderAlertEquals(request.id) &&
          !_alertedOrderRequestIds.contains(request.id)) {
  ...
  /// Quick-access language switcher for the dashboard's language icon.
  /// Mirrors `AgentSettingsScreen._openLanguagePicker` exactly (same enum,
  /// same `AgentService.updateSettings` call) so the two entry points never
  /// disagree about which language is active.
  Future<void> _openLanguagePicker() async {
    // Refresh from the server first in case it was changed elsewhere
    // (e.g. the full Settings screen, or another device).
    try {
      final settings = await _agentService.getSettings(widget.user.id,
          token: widget.user.token ?? '');
      if (mounted) {
        setState(() => _language = languageFromString(settings.language));
      }
    } on AgentServiceException {
      // Fall back to the last-known value; the picker below still works.
    }
            ...
            for (final lang in AppLanguage.values)
              RadioListTile<AppLanguage>(
                ...
                onChanged: (v) async {
                  ...
                  try {
                    await _agentService.updateSettings(
                      widget.user.id,
                      language: v.apiValue,
                      token: widget.user.token ?? '',
                    );
  ...
  bool _pendingOrderAlertEquals(String requestId) {
    return _pendingOrderAlert?.id == requestId;
  }

  void _showClaimOrderSheet(BuildContext context, OrderRequest request) {
    // Show full-screen order alert instead of bottom sheet
    _alertedOrderRequestIds.add(request.id);
    setState(() {
      _pendingOrderAlert = request;
      _orderAlertSecondsLeft = 30;
      _orderAlertCountdown?.cancel();
      _orderAlertCountdown = Timer.periodic(const Duration(seconds: 1), (t) {
        if (!mounted) {
          t.cancel();
          return;
        }
        if (_orderAlertSecondsLeft <= 1) {
          setState(() {
            _orderAlertSecondsLeft = 0;
            _pendingOrderAlert = null;
          });
          t.cancel();
        } else {
          setState(() => _orderAlertSecondsLeft--);
        }
      });
    });
  }

  void _dismissOrderAlert() {
    setState(() => _pendingOrderAlert = null);
    _orderAlertCountdown?.cancel();
  }
  ...
  Future<void> _setMyLocation() async {
    ...
    try {
      if (!await Geolocator.isLocationServiceEnabled()) {
        if (mounted) {
          setState(() =>
              _locationError = 'Location is turned off on this device.');
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text(
                  'Location is turned off. Turn it on to go online.'),
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
                        ...
                        onPropertyManagement: () => Navigator.of(context).push(
                            MaterialPageRoute(
                                builder: (_) => AgentSellRequestsScreen(
                                    user: widget.user))),
                        onMyListings: () => Navigator.of(context).push(
                            MaterialPageRoute(
                                builder: (_) =>
                                    AgentListingsScreen(user: widget.user))),
            ...
            // Real incoming-tour-request alert. Gated to fire once per
            // request id (see _alertedOrderRequestIds / armExpiry on the
            // backend) so it no longer re-pops for the same request.
            if (loop.stage == LoopStage.dispatched)
              _RingingOverlay(loop: loop, agentId: widget.user.id),
            if (_pendingOrderAlert != null)
              _OrderAlertOverlay(
                order: _pendingOrderAlert!,
                secondsLeft: _orderAlertSecondsLeft,
                agentId: widget.user.id,
                onDismiss: _dismissOrderAlert,
                onClaim: () {
                  _dismissOrderAlert();
                  _claim(_pendingOrderAlert!);
                },
              ),
          ],
        ),
...
class _OrderRequestsSection extends StatelessWidget {
  ...
  /// The "set your location" nudge only concerns *new* nearby broadcasts —
  /// it has nothing to do with requests this agent already claimed. This
  /// used to gate the whole section (including "Your claimed requests"),
  /// which meant an agent with no location on file couldn't see anything
  /// they'd already claimed, even though the claim succeeded server-side.
  /// Now it only replaces the "near you" half.
  ...
  @override
  Widget build(BuildContext context) {
    final showLocationPrompt = !hasLocation;

    if (!showLocationPrompt && available.isEmpty && assigned.isEmpty) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (showLocationPrompt) _buildLocationPrompt(),
        if (!showLocationPrompt && available.isNotEmpty) ...[
          _CollapsibleRequestsSection(
            title: 'Order requests near you',
            icon: Icons.near_me_outlined,
            requests: available,
            collapsedHint: 'Tap to view nearby requests',
        ...
        if (assigned.isNotEmpty) ...[
          if (showLocationPrompt) const SizedBox(height: AppSpacing.sm),
          if (!showLocationPrompt && available.isNotEmpty)
            const SizedBox(height: AppSpacing.sm),
          _CollapsibleRequestsSection(
            ...
            requests: assigned,
            collapsedHint: 'Tap to view locations and inspection details',
            expandedHint: 'Tap to collapse property details',
          ),
        ],
      ],
    );
...
/// A collapsible list of [OrderRequest] cards behind a tap-to-expand
/// header — collapsed by default so a long list of leads doesn't push
/// the rest of the tab down. Used for both the claimable "near you" list
/// (with a Claim button per card) and the agent's own claimed requests
/// (read-only).
class _CollapsibleRequestsSection extends StatefulWidget {
  const _CollapsibleRequestsSection({
    required this.title,
    required this.icon,
  ...
  @override
  State<_CollapsibleRequestsSection> createState() =>
      _CollapsibleRequestsSectionState();
}

class _CollapsibleRequestsSectionState
    extends State<_CollapsibleRequestsSection> {
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('${widget.title} (${widget.requests.length})',
                            style: const TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.w800,
                                color: AppColors.ink)),
                        const SizedBox(height: 2),
                        Text(
                          _expanded ? widget.expandedHint : widget.collapsedHint,
                          style: const TextStyle(
                ...
                children: [
                  for (final request in widget.requests) ...[
                    _OrderRequestCard(
                      ...
                      onPrimary: widget.onPrimary == null
                          ? null
                          : () => widget.onPrimary!(request),
                    ),
                    const SizedBox(height: AppSpacing.sm),
                  ],
                ],
...
class _OrderAlertOverlay extends StatelessWidget {
  const _OrderAlertOverlay({
    required this.order,
        ...
        child: SafeArea(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 6),
              Text(
                '${order.budgetSummary} · ${order.locationSummary}',
                textAlign: TextAlign.center,
                style: const TextStyle(
                  color: Color(0xFFB9B8AE),
                  fontSize: 13.5,
                ),
              ),
              const SizedBox(height: 12),
