...
class AdminListingDetailScreen extends StatefulWidget {
  const AdminListingDetailScreen({super.key, required this.asset, required this.token});

  final Asset asset;

  /// Admin JWT. `PATCH`/`DELETE /api/assets/:id` are both behind
  /// requireAuth + requireAdmin server-side, so this is not optional —
  /// without it every save/delete comes back 401.
  final String token;
...
class _AdminListingDetailScreenState extends State<AdminListingDetailScreen> {
  ...
  Future<void> _deleteListing() async {
    ...
    try {
      await _assetService.deleteAsset(widget.asset.id, token: widget.token);
      if (!mounted) return;
...
class _StatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final color = switch (status) {
      ...
      AssetStatus.reserved => AppColors.primaryYellowDark,
      AssetStatus.rented => AppColors.slate,
    };
    return Container(
