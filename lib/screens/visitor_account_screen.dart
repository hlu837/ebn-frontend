...
class VisitorAccountScreen extends StatelessWidget {
  ...
  Future<void> _logout(BuildContext context) async {
    // Catch the session on the way out too: clear whatever was persisted
    // at login/signup so a restart of the app doesn't silently sign this
    // device back in as this visitor.
          ...
          child: Column(
            ...
            children: [
              ...
              _MenuTile(
                ...
                label: 'My Rental Agreements',
                subtitle: 'Documents under review, agreements to pay, and closed rentals',
                onTap: () => Navigator.of(context).push(MaterialPageRoute(
                  builder: (_) => MyRentalAgreementsScreen(user: user),
                )),
              ),
              // Only shown once the caller actually has at least one active
              // lease — same check maintenanceRequests.create re-verifies
              // server-side on submit. A FutureBuilder here (rather than
              // converting this whole screen to Stateful) since it's the
              // one row on this menu that needs a network check first.
              FutureBuilder<bool>(
                future: MaintenanceRequestService()
                    .fetchEligibleAssets(token: user.token ?? '')
                ...
                builder: (context, snapshot) {
                  ...
                  return _MenuTile(
                    ...
                    subtitle: 'Issues you\'ve reported on your active lease',
                    onTap: () => Navigator.of(context).push(MaterialPageRoute(
                      builder: (_) => MyMaintenanceRequestsScreen(user: user),
                    )),
                  );
                },
              ),
              _MenuTile(
                ...
                subtitle: 'Local electricians, plumbers, and other specialists',
                onTap: () => Navigator.of(context).push(MaterialPageRoute(
                  builder: (_) => ServiceProviderDirectoryScreen(user: user),
                )),
              ),
