...
class AffiliateAccountSettingsScreen extends StatefulWidget {
  const AffiliateAccountSettingsScreen({
    super.key,
    required this.user,
  ...
  /// Called with the fresh [AppUser] right after the name/phone PATCH
  /// succeeds. The caller (typically the role's home screen) uses this to
  /// swap its own copy of the user in, since there's no global auth store
  /// this screen can write to directly. Optional so this screen still
  /// works if a caller doesn't care to track it.
...
class _AffiliateAccountSettingsScreenState extends State<AffiliateAccountSettingsScreen> {
  ...
  Future<void> _save() async {
    final token = _token();
    if (token == null) {
      AppToast.showError(context, 'You are not logged in. Please sign in again.');
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      final updatedUser = results[0] as AppUser;
      ...
      // The PATCH /api/auth/me call above already persisted the new
      // name/phone server-side — this just lets whoever pushed this
      // screen know, since `widget.user` here is only a snapshot passed
      // in at navigation time and won't update on its own.
