...
class _SubmitRentalDocumentsScreenState extends State<SubmitRentalDocumentsScreen> {
  ...
  Future<void> _submit() async {
    ...
    final faydaId = _faydaIdController.text.trim();
    if (faydaId.length != 12) {
      setState(() => _error = 'Enter your 12-digit Fayda ID number.');
      return;
    }
    setState(() {
      _submitting = true;
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: AppSpacing.lg),
            const Text('Fayda ID number *', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.ink)),
            const SizedBox(height: 4),
            const Text(
              'Your 12-digit National ID (Fayda) number.',
