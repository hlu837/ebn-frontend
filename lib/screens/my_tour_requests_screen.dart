              ...
              : _error != null
                  ...
                  : _rows.isEmpty
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (context, i) => _TourRow(
                            row: _rows[i],
                            forAgent: widget.forAgent,
...
class _TourRow extends StatefulWidget {
  ...
  /// Only needed (and only used) when [forAgent] is true, so this row can
  /// claim a `broadcasting` request itself instead of sending the agent
  /// back to the Home tab's banner — same `POST /:id/claim` call, same
  /// first-come-first-served race, just fired from here.
  final String? agentId;
  final String? agentName;

  /// Called after a successful claim so the parent can refresh the list
  /// (the row's status will have flipped away from `broadcasting`).
...
class _TourRowState extends State<_TourRow> {
  ...
  bool get forAgent => widget.forAgent;

  Future<void> _claim() async {
    setState(() => _claiming = true);
    final loop = context.read<LoopController>();
    final ok = await loop.agentClaim(
      row['id'] as String,
    ...
    } else if (loop.lastError != null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text(loop.lastError!)));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final canClaim = forAgent && row['status'] == 'broadcasting';
