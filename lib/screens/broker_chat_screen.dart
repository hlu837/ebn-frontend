...
import '../utils/network_alert.dart';
...
class BrokerChatScreen extends StatefulWidget {
  /// The three ways into this screen:
  ...
  ///  - [broker] with no [asset] (via [BrokerChatScreen.general]): from a
  ///    broker's profile "Chat" button when they have no active listings
  ///    to anchor a per-listing thread to — opens/reuses the one general
  ///    thread between this customer and that broker.
  ...
  const BrokerChatScreen.general({
    super.key,
    required Broker this.broker,
    required this.currentUser,
  })  : asset = null,
...
class _BrokerChatScreenState extends State<BrokerChatScreen> {
  ...
  String get _assetTitle =>
      widget.asset?.title ?? _thread?.asset?.title ?? 'General inquiry';
    ...
    try {
      final thread = widget.initialThread ??
          (widget.asset != null
              ...
              : await _service.openThreadForBroker(
                  token: _token, brokerId: widget.broker!.id));
      final history =
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, i) {
                            ...
                            return _Bubble(
                              message: m,
                              mine: mine,
                              // Only the recipient of an agreement message
                              // (the tenant) gets the button — the owner
                              // who sent it manages it from their own
                              // review queue instead.
                              onOpenAgreement: !mine &&
                                      m.kind == api.ChatMessage.kindRentalAgreement &&
                                      m.relatedId != null
                                  ? () => Navigator.of(context).push(
                                        MaterialPageRoute(
                                          builder: (_) => MyRentalAgreementsScreen(
                                            user: widget.currentUser,
                                            openAgreementId: m.relatedId,
                                          ),
                                        ),
                                      )
                                  : null,
...
class _Bubble extends StatelessWidget {
  ...
  /// When non-null, an "Open agreement" button is shown under the text.
  final VoidCallback? onOpenAgreement;
  const _Bubble({required this.message, required this.mine, this.onOpenAgreement});

  @override
  Widget build(BuildContext context) {
    final bg = mine ? LandingColors.foreground : LandingColors.card;
    final fg = mine ? LandingColors.primaryFg : LandingColors.foreground;
    // A rental-agreement message that carries its figures is drawn as a
    // summary card; anything else (including agreement messages from
    // before this existed) falls back to the plain text body.
    final meta = message.meta;
    final isAgreementCard = message.kind == api.ChatMessage.kindRentalAgreement &&
        meta != null &&
        meta['totalDue'] != null;
    return Align(
      ...
      child: Container(
        ...
        constraints:
            BoxConstraints(maxWidth: MediaQuery.of(context).size.width * (isAgreementCard ? 0.86 : 0.72)),
        decoration: BoxDecoration(
          color: bg,
        ...
        child: Column(
          ...
          children: [
            if (isAgreementCard)
              _AgreementSummaryCard(meta: meta, fg: fg)
            else
              Text(message.body,
                  style: TextStyle(fontSize: 14, color: fg, height: 1.3)),
            if (onOpenAgreement != null) ...[
              ...
              SizedBox(
                ...
                child: FilledButton.icon(
                  ...
                  label: const Text('Open agreement'),
                  style: FilledButton.styleFrom(
                    backgroundColor: LandingColors.foreground,
                    foregroundColor: LandingColors.primaryFg,
...
/// 1234567.5 -> "1,234,567.50", 225000 -> "225,000".
String _money(num v) {
...
/// The organised version of the "Rental agreement sent" message: every
/// figure comes straight from the server's own calculation (listing price x
/// advance months, plus deposit), so what's shown here is exactly what the
/// tenant is asked to pay.
class _AgreementSummaryCard extends StatelessWidget {
  final Map<String, dynamic> meta;
  final Color fg;
  const _AgreementSummaryCard({required this.meta, required this.fg});

  @override
  Widget build(BuildContext context) {
    final cur = (meta['currency'] as String?) ?? 'ETB';
    final rent = meta['monthlyRent'] as num?;
    final months = (meta['advanceMonths'] as num?)?.toInt();
    final advance = meta['advanceAmount'] as num?;
    final deposit = meta['depositAmount'] as num?;
    final total = meta['totalDue'] as num?;
    final term = (meta['leaseTermMonths'] as num?)?.toInt();
    final hours = (meta['windowHours'] as num?)?.toInt();

    final muted = fg.withValues(alpha: 0.65);
    final line = fg.withValues(alpha: 0.14);
    ...
    Widget row(String label, String value) => Padding(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Expanded(child: Text(label, style: TextStyle(fontSize: 13, color: muted, height: 1.3))),
              const SizedBox(width: 12),
              Flexible(
                child: Text(value,
                    textAlign: TextAlign.right,
                    style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w700, color: fg, height: 1.3)),
              ),
            ],
          ),
        );
    ...
    return Column(
      ...
      children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text('Standard lease agreement generated',
        ...
        if (rent != null) row('Monthly rent', '${_money(rent)} $cur'),
        if (advance != null)
          row(months != null ? 'Advance rent · $months month${months == 1 ? '' : 's'}' : 'Advance rent',
              '${_money(advance)} $cur'),
        if (deposit != null) row('Deposit', '${_money(deposit)} $cur'),
        row('Lease term', term != null ? '$term month${term == 1 ? '' : 's'}' : 'Month-to-month'),
        const SizedBox(height: 6),
        Divider(height: 1, color: line),
        ...
        if (total != null)
          Row(
            children: [
              Expanded(child: Text('Total due', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: fg))),
              Text('${_money(total)} $cur',
        ...
        if (hours != null) ...[
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Text('Pay within ${hours}h to confirm',
                    style: TextStyle(fontSize: 12, color: muted)),
              ),
            ],
          ),
