...
class _AgentListingEditScreenState extends State<AgentListingEditScreen> {
  ...
  /// Renders an existing listing photo, whether it's a real hosted URL or
  /// a base64 `data:` URL saved by a previous edit — `Image.network`
  /// cannot load `data:` URIs on Android/iOS (it only performs real HTTP
  /// requests), so those must go through `Image.memory` instead.
  ///
  /// The decode is pushed onto a background isolate via `compute()` and
  /// awaited through a `FutureBuilder`, rather than done inline during
  /// `build()`: a listing can have several of these embedded photos, each
  /// several MB of base64 text, and `GridView.builder` below uses
  /// `shrinkWrap: true` (so every tile builds at once, not lazily as you
  /// scroll) — decoding all of them synchronously on the UI thread in a
  /// single frame was blocking the whole screen (no repaint, no touch
  /// handling — including the back button) for as long as that took.
  Widget _buildExistingImage(String url) {
    if (url.startsWith('data:')) {
      final base64Part = url.substring(url.indexOf(',') + 1);
      return FutureBuilder<Uint8List>(
        future: compute(base64Decode, base64Part),
  ...
  /// Wrapped in try/catch so that in a release build (no dev tools
  /// attached) a bug here shows an actual error message instead of
  /// silently rendering as a blank screen.
  Widget _buildBody(BuildContext context) {
    try {
      return _buildForm(context);
    } catch (e, stack) {
      debugPrint('AgentListingEditScreen build error: $e\n$stack');
      return Center(
        child: Padding(
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  hintText: '0.00',
                  prefixText: '${widget.asset.priceCurrency} ',
                ),
                validator: (v) {
                  if (v == null || v.trim().isEmpty) {
                      ...
                      children: [
                        Container(
                          ...
                          child: ClipRRect(
                            borderRadius: BorderRadius.circular(AppRadii.md),
                            child: isString
                                ? _buildExistingImage(imageUrlOrFile as String)
                                : Image.file(
