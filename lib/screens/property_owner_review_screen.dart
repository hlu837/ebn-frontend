...
class _PropertyOwnerReviewScreenState extends State<PropertyOwnerReviewScreen> {
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    try {
      ...
      widget.onQueueChanged?.call(active
          .where((r) => r.status == RentalAgreementStatus.documentsSubmitted || r.status == RentalAgreementStatus.paymentSubmitted)
          .length);
      ...
      } else if (_filter == _Filter.awaitingPayment) {
        // Both agreement_sent (terms sent, not yet accepted) and accepted
        // (terms accepted, payment still pending) count as "awaiting
        // payment" from the owner's point of view — same countdown either way.
        rows = active
            .where((r) => r.status == RentalAgreementStatus.agreementSent || r.status == RentalAgreementStatus.accepted)
            .toList();
      } else {
        // needsReview: anything that requires an owner decision right now
        // — a fresh submission, or a tenant-submitted receipt awaiting
...
class _ReviewCard extends StatelessWidget {
  ...
  Color get _statusColor {
    switch (agreement.status) {
      ...
      case RentalAgreementStatus.paid:
        return agreement.vacatedAt != null ? AppColors.slate : AppColors.success;
  ...
  String get _subtitle {
    switch (agreement.status) {
      ...
      case RentalAgreementStatus.agreementSent:
        return agreement.timeLeft != null ? _formatCountdown(agreement.timeLeft!) : 'Awaiting acceptance';
      case RentalAgreementStatus.accepted:
        return agreement.timeLeft != null ? _formatCountdown(agreement.timeLeft!) : 'Accepted — awaiting payment';
      case RentalAgreementStatus.paymentSubmitted:
        return 'Receipt submitted — needs your review';
      case RentalAgreementStatus.paid:
        if (agreement.vacatedAt != null) return 'Vacated — listing reopened';
        if (agreement.leaseEndAt != null) return 'Paid by ${agreement.requester?.fullName ?? 'tenant'} — lease ends ${_formatDate(agreement.leaseEndAt!)}';
...
class _EmptyState extends StatelessWidget {
  ...
  String get _message {
    switch (filter) {
      case _Filter.needsReview:
        return "Nothing needs your review right now — new document submissions and payment receipts will show up here.";
      case _Filter.awaitingPayment:
