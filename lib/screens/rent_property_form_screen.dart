...
class RentPropertyFormScreen extends StatefulWidget {
  const RentPropertyFormScreen(
      {super.key, required this.user, this.isAgentListing = false});

  final AppUser user;

  /// Agents/brokers have active subscriptions and don't pay a per-listing
  /// fee — mirrors [SellPropertyFormScreen.isAgentListing].
...
class _RentPropertyFormScreenState extends State<RentPropertyFormScreen> {
  ...
  // Property wizard: 6. Media Upload — the property side never had a
  // photo step (unlike the vehicle side above), so real submission had
  // nowhere to get media from. Mirrors the vehicle fields exactly.
  final List<ReportMediaItem> _propertyMedia = [];
  int _propertyMediaMockId = 1;
  ...
  void _addPropertyMedia() async {
    try {
      final encoded = await pickAndEncodeImage();
      if (encoded != null) {
        setState(() => _propertyMedia.add(ReportMediaItem(
          id: 'pm${_propertyMediaMockId++}',
          filePath: encoded,
        )));
      }
    } on FormatException catch (e) {
  ...
  void _removePropertyMedia(String id) {
    setState(() => _propertyMedia.removeWhere((m) => m.id == id));
  }
  ...
  Future<void> _submit() async {
    ...
    // A short auto-note stands in for the "Notes" field the Sell flow's
    // agent-listing path has — the rent wizard has no equivalent field,
    // and the backend requires a non-empty value for `notes`.
    const autoNotes = 'Submitted directly by the property owner via the app.';

    try {
      if (_isVehicle) {
        final details = _collectVehicleDetails();
        final title = '${_makeModelController.text.trim()} — for rent';
        final askingPrice = details.monthlyRateEtb ?? details.dailyRateEtb;
        const city = 'Addis Ababa';
        final addressLine = widget.isAgentListing
            ? 'Vehicle handed over directly by the owner'
            : 'Vehicle location shared with assigned broker';
        if (widget.isAgentListing) {
          await controller.submitAsAgent(
        ...
        final title = '${details.bedrooms}-bedroom ${details.propertyType.label} for rent'
            '${details.zoneSubCity.isNotEmpty ? ' in ${details.zoneSubCity}' : ''}';
        final askingPrice = details.monthlyRentEtb;
        final city = details.zoneSubCity.isNotEmpty ? details.zoneSubCity : 'Addis Ababa';
        final addressLine = details.landmark.isNotEmpty
            ? 'Near ${details.landmark}'
            : (widget.isAgentListing
                ? 'Address on file with the property owner'
  ...
  Future<bool?> _showPaymentSheet() {
    return showModalBottomSheet<bool>(
      ...
      builder: (sheetContext) => _RentPaymentSheet(amount: _kListingFeeEtb, user: widget.user),
    );
  }
  ...
  Widget _buildStep() {
    ...
    switch (_step) {
      ...
      case 6:
        return _buildPropertyMediaStep();
      default:
  ...
  // ── Review & submit ─────────────────────────────────────────────────
  // ── 6. Media Upload ──────────────────────────────────────────────────
  Widget _buildPropertyMediaStep() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const _StepHeader(
          title: 'Media Upload',
          subtitle: 'Photos of the property — exterior, living areas, kitchen, and bedrooms.',
        ),
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: [
            for (final item in _propertyMedia) _MediaThumb(item: item, onRemove: () => _removePropertyMedia(item.id)),
            _AddMediaButton(icon: Icons.add_a_photo_outlined, label: 'Photo', onTap: _addPropertyMedia),
          ],
        ),
      ],
    );
      ...
      children: [
        _StepHeader(
          ...
          subtitle: widget.isAgentListing
              ...
              : 'Double check the details below, then pay the one-time ETB 100 review fee to send this to our team.',
        ),
        Container(
...
/// Real Chapa checkout for the rent-listing review fee. Mirrors
/// [SellPropertyFormScreen]'s `_PaymentSheet`: hits the backend's
/// `/api/payments/chapa/*` routes, opens the hosted checkout page in the
/// system browser, then polls `verify` until the backend confirms the
/// payment with Chapa directly.
class _RentPaymentSheet extends StatefulWidget {
  const _RentPaymentSheet({required this.amount, required this.user});
  final double amount;
  final AppUser user;
...
enum _RentPayStage { idle, launching, waiting, verifying, failed }

class _RentPaymentSheetState extends State<_RentPaymentSheet> {
  final _service = PaymentService();
  _RentPayStage _stage = _RentPayStage.idle;
  String? _txRef;
  String? _error;
  Timer? _poll;
  int _pollAttempts = 0;
  ...
  Future<void> _startCheckout() async {
    setState(() {
      _stage = _RentPayStage.launching;
      _error = null;
    });
    final (firstName, lastName) = _splitName();
    try {
      final checkout = await _service.initialize(
        purpose: 'rent_request_fee',
        amount: widget.amount,
        email: widget.user.email,
        ownerUserId: widget.user.id,
        firstName: firstName,
        lastName: lastName.isEmpty ? null : lastName,
        description: 'Onsite rental listing review fee',
      );
      ...
      if (!opened) {
        setState(() {
          _stage = _RentPayStage.failed;
          _error = "Couldn't open the payment page. Try again.";
        });
        return;
      }

      setState(() => _stage = _RentPayStage.waiting);
      _pollAttempts = 0;
  ...
  Future<void> _checkStatus({bool manual = false}) async {
    ...
    if (manual) {
      _poll?.cancel();
      setState(() => _stage = _RentPayStage.verifying);
    }
    _pollAttempts++;
    try {
      ...
      // Still pending.
      if (manual) setState(() => _stage = _RentPayStage.waiting);
        ...
        children: [
          Row(
            children: [
              ...
              if (_stage == _RentPayStage.idle || _stage == _RentPayStage.failed)
