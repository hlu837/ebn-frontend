...
/// Property Owner is web-only: the role's workspace only exists in the
/// web build (see `role_router_mobile.dart`), so offering it as a sign-up
/// choice on mobile would land someone on the "use the web app" placeholder
/// right after registering. `kIsWeb` is a runtime check (not the
/// `dart.library.html` compile-time conditional export used in
/// `role_router.dart`), which is the right tool here since this list is a
/// handful of enum values, not a screen subtree worth tree-shaking out —
/// both platforms compile this file, they just render a different list.
List<UserRole> get _selectableRoles => [
      UserRole.user,
      UserRole.affiliater,
      UserRole.agent,
      UserRole.investor,
      if (kIsWeb) UserRole.propertyOwner,
    ];
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "Pick how you'll use EBN — you can always add another role later.",
...
class _RoleCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      color: selected ? AppColors.primaryYellow.withValues(alpha: 0.1) : AppColors.card,
              ...
              Column(
                ...
                children: [
                  Container(
                    ...
                    child: Icon(role.pitchIcon, color: const Color(0xFF4A4A45), size: 20),
                  ),
