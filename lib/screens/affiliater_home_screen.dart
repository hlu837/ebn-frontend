...
import 'affiliate_expert_screen.dart';
...
class _AffiliaterHomeScreenState extends State<AffiliaterHomeScreen> {
  // A local, mutable copy of the signed-in user. The app has no global
  // auth/user store — `AppUser` is just threaded through screens via
  // constructor params — so when the Account Settings screen saves a new
  // name/phone, it hands the fresh `AppUser` back here via
  // `onProfileUpdated` and we swap it in. Every reference to the user
  // below reads `_user`, not `widget.user`, so this screen (and whatever
  // it pushes) reflects the edit immediately instead of showing stale
  // data until the app is restarted.
  ...
  bool _topPropertiesLoading = true;
  bool _topPropertiesFailed = false;
  ...
  Future<void> _loadTopProperties() async {
    setState(() {
      _topPropertiesLoading = true;
      _topPropertiesFailed = false;
    });
    try {
      ...
      setState(() {
        ...
        _topPropertiesLoading = false;
      });
    } on AssetException catch (_) {
      // Previously this left the list empty with zero feedback, so a real
      // fetch failure looked identical to "no listings yet" — an empty
      // gap with no explanation. Now it shows the same retry affordance
      // AffiliatePropertiesScreen's "View All" uses for the same error.
      if (!mounted) return;
      setState(() {
        _topPropertiesLoading = false;
        _topPropertiesFailed = true;
      });
    }
  }
  ...
  Future<void> _loadAffiliateData() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _affiliateCode = 'EBN-${_user.id.substring(0, 6).toUpperCase()}';
      });
    } finally {
      ...
      builder: (_) => _MoreBottomSheet(
        onReports: () {
          ...
          Navigator.of(context).push(MaterialPageRoute(
              builder: (_) =>
                  AffiliateReportsScreen(token: _user.token ?? '')));
        },
        onMembership: () {
          Navigator.pop(context);
          Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => AffiliateMembershipScreen(user: _user)));
        },
        onExpert: () {
          Navigator.pop(context);
          Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => AffiliateExpertScreen(user: _user)));
        },
        onSettings: () {
          Navigator.pop(context);
          Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => AffiliateAccountSettingsScreen(
                    user: _user,
                    // Fired as soon as the name/phone PATCH succeeds (not
                    // just when the screen closes), so this home screen —
                    // and anything else pushed with `_user` afterward —
                    // picks up the change right away instead of showing
        ...
        onSupport: () {
          Navigator.pop(context);
          Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => AffiliateSupportScreen(user: _user)));
        },
        onLogout: () {
          Navigator.pop(context);
          _logout();
    ...
    return Scaffold(
                ...
                children: [
                  ...
                  _QuickActionsRow(
                    ...
                    onAssets: () => Navigator.of(context).push(
                        MaterialPageRoute(
                            builder: (_) =>
                                AffiliatePropertiesScreen(user: _user))),
                    onCampaigns: () => Navigator.of(context).push(
                        MaterialPageRoute(
                            builder: (_) =>
                                AffiliateCampaignsScreen(user: _user))),
                            ...
                            : _topProperties.isEmpty
                                ...
                                : ListView.separated(
                                    ...
                                    itemBuilder: (context, i) => _AffiliatePropertyCard(
                                      ...
                                      onGenerateLink: () =>
                                          _generateLink(_topProperties[i]),
                                    ),
                                  ),
                  ),
                  const SizedBox(height: AppSpacing.xl),
      ...
      bottomNavigationBar: _BottomNavBar(
        ...
        onChanged: (i) {
          if (i == 1) {
            Navigator.of(context).push(MaterialPageRoute(
                builder: (_) => AffiliatePropertiesScreen(user: _user)));
            return;
          }
          if (i == 2) {
            Navigator.of(context).push(MaterialPageRoute(
        ...
        child: Column(
          ...
          children: [
            ...
            _MoreTile(
              ...
              iconFilled: Icons.engineering_rounded,
              label: 'Expert',
              subtitle: 'Specialist profile, jobs & payouts',
