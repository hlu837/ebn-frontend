...
class _VerificationPendingScreenState extends State<VerificationPendingScreen> {
  ...
  /// Only meaningful when [widget.targetRole] is [UserRole.propertyOwner]:
  /// an admin rejection demotes the account straight back to an active
  /// plain Visitor (see `rejectPendingRole` in the backend's users model)
  /// rather than leaving a rejected request behind to poll for, so this
  /// is set locally once [_checkStatus] notices the account is no longer
  /// `pending_approval` but also never became [UserRole.propertyOwner].
  bool _propertyOwnerRejected = false;

  bool get _isPropertyOwnerFlow => widget.targetRole == UserRole.propertyOwner;
  ...
  Future<void> _checkStatus({bool silent = false}) async {
    ...
    try {
      ...
      // Property Owner has no RoleUpgradeRequest row to poll (approval
      // lives on the user row itself), so it's handled on its own here:
      // still `pending_approval` means still waiting; anything else means
      // an admin rejected it and the backend silently demoted the account
      // back to an active plain Visitor (see `rejectPendingRole`).
      if (_isPropertyOwnerFlow) {
        if (mounted) {
          setState(() {
            _propertyOwnerRejected = updatedUser.accountStatus != 'pending_approval';
            _checking = false;
          });
        }
        return;
      ...
      // 2. Fetch latest request status — the Agent/Investor "Visitor
      // requests an upgrade" flow.
  ...
  Widget build(BuildContext context) {
    ...
    final isRejected = _isPropertyOwnerFlow
        ? _propertyOwnerRejected
        : req != null && req.status == RoleUpgradeStatus.rejected;
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text(
                  _isPropertyOwnerFlow
                      ? (isRejected
                          ? "An admin didn't approve your Property Owner account. You're still signed in as a Visitor — you can browse listings normally, or reach out to support if you think this was a mistake."
                          : 'An admin is reviewing your Property Owner account — no payment needed. This usually takes 12–24 hours.')
                      : (isRejected
              ...
              if (!isRejected)
                Container(
                  ...
                  child: Column(
                    children: [
                      _TimelineStep(
                        title: _isPropertyOwnerFlow ? 'Account Created' : 'Receipt Submitted',
                        subtitle: _isPropertyOwnerFlow
                            ? 'Your Property Owner account is on file'
                            : (req != null
                      ...
                      _TimelineStep(
                        title: 'Admin Review',
                        subtitle: _isPropertyOwnerFlow
                            ? 'Checking your details — no payment involved'
                            : 'Reviewing transfer reference and receipt copy',
                        isActive: _isPropertyOwnerFlow || (isSubmitted && isPending),
                        isCompleted: false,
                        showLine: true,
                      ),
                      _TimelineStep(
              ...
              if (isRejected && _isPropertyOwnerFlow) ...[
                Container(
                  ...
                  child: const Text(
                    "You still have a full account and can browse, request tours, or rent as a Visitor in the meantime.",
                ...
                SizedBox(
                  ...
                  child: PrimaryButton(
                    ...
                    onPressed: () {
                      Navigator.of(context).pushAndRemoveUntil(
                        MaterialPageRoute(
                          builder: (_) => dashboardForRole(UserRole.user, widget.user),
                        ),
                        (route) => false,
                      );
                    },
