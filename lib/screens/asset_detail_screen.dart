...
import '../utils/screenshot_guard.dart';
import 'broker_chat_screen.dart';
import 'broker_profile_screen.dart';
import 'submit_rental_documents_screen.dart';
          ...
          children: [
            ...
            FilledButton.icon(
              onPressed: () {
                ...
                WidgetsBinding.instance
                    .addPostFrameCallback((_) => _makePhoneCall(phoneNumber));
              },
            ...
            FilledButton.icon(
              onPressed: () {
                Navigator.pop(context);
                WidgetsBinding.instance
                    .addPostFrameCallback((_) => _sendSMS(phoneNumber));
              },
              style: FilledButton.styleFrom(
                backgroundColor: AppColors.primaryYellow,
                foregroundColor: Colors.white,
            ...
            FilledButton.icon(
              onPressed: () {
                Navigator.pop(context);
                WidgetsBinding.instance
                    .addPostFrameCallback((_) => _handleTourRequest());
              },
              style: FilledButton.styleFrom(
                backgroundColor: AppColors.primaryYellow,
                foregroundColor: Colors.white,
...
class _ContactSectionState extends State<_ContactSection> {
  ...
  Widget build(BuildContext context) {
    ...
    if (broker != null) {
      return _BrokerSection(broker: broker, asset: widget.asset, currentUser: widget.currentUser);
    }
    // No agent resolved for this listing's broker_id — it may still be a
    // Property Owner's self-listing (agents-directory lookup above only
    // ever returns agents), so try that before falling back to the EBN
    // office card. The request call itself is the real check: it 422s
    // with a clear message if brokerId doesn't resolve to anyone.
    if (widget.asset.brokerId != null) {
      return _OwnerContactSection(asset: widget.asset, currentUser: widget.currentUser);
    }
    return const _OfficeSection();
  }
}

/// Shown when the listing has no assigned agent but does have a
/// `broker_id` — the renter-facing entry point into a Property Owner's
/// Inbox (see `property_owner_inbox_screen.dart`). Each button opens (or
/// reuses) a chat thread with the owner and files it under that request
/// type; sending fails gracefully with the server's own message if
/// `broker_id` doesn't actually resolve to an owner.
class _OwnerContactSection extends StatefulWidget {
  const _OwnerContactSection({required this.asset, required this.currentUser});

  final Asset asset;
  final AppUser currentUser;

  @override
  State<_OwnerContactSection> createState() => _OwnerContactSectionState();
}

class _OwnerContactSectionState extends State<_OwnerContactSection> {
  final _service = PropertyRequestService();
  bool _sending = false;

  String get _token => widget.currentUser.token ?? '';

  Future<void> _requestInfo() => _send(PropertyRequestType.info, 'Ask a question',
      hint: 'e.g. Is this still available? Any pets allowed?');

  Future<void> _requestTour() => _send(PropertyRequestType.tour, 'Request a tour',
      hint: 'e.g. Could I see this Saturday afternoon?');

  Future<void> _rentNow() => _send(PropertyRequestType.rentNow, 'Rent this property',
      hint: "I'd like to rent this property.");

  Future<void> _send(PropertyRequestType type, String dialogTitle, {required String hint}) async {
    final message = await _composeMessage(context, title: dialogTitle, hint: hint);
    if (message == null || message.trim().isEmpty || !mounted) return;

    setState(() => _sending = true);
    try {
      final (request, thread) = await _service.create(
        token: _token,
      ...
      if (type == PropertyRequestType.rentNow) {
        // Rent requests continue into the digital-ID/documents step
        // rather than straight into chat — chat is still there for
        // anything else, from the property owner's Inbox/Review tabs or
        // via "Ask a question" above.
        Navigator.of(context).push(MaterialPageRoute(
          builder: (_) => SubmitRentalDocumentsScreen(
      ...
      Navigator.of(context).push(MaterialPageRoute(
        builder: (_) => BrokerChatScreen.fromThread(thread: thread, currentUser: widget.currentUser),
      ));
  ...
  Future<String?> _composeMessage(BuildContext context, {required String title, required String hint}) {
    // Always starts blank — no canned/template text to clear first, so
    // the visitor writes their message to the property owner from
    // scratch every time.
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Contact the property owner',
            style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.slate)),
        const SizedBox(height: AppSpacing.sm),
        const Text(
          'This listing is rented directly by its owner — no physical visit needed until you\'re ready.',
          ...
          children: [
            Expanded(
              child: OutlinedButton(
                onPressed: _sending ? null : _requestInfo,
                style: OutlinedButton.styleFrom(
            ...
            Expanded(
              child: OutlinedButton(
                onPressed: _sending ? null : _requestTour,
                style: OutlinedButton.styleFrom(
                  minimumSize: const Size(0, 40),
                  side: const BorderSide(color: AppColors.ink, width: 1.2),
                  textStyle: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700),
                ),
                child: const Text('Request a tour'),
              ),
            ),
          ],
        ),
        ...
        SizedBox(
          width: double.infinity,
          child: FilledButton(
            onPressed: _sending ? null : _rentNow,
            style: FilledButton.styleFrom(
              backgroundColor: AppColors.primaryYellow,
              foregroundColor: Colors.white,
