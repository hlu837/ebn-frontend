...
class OrderRequestController extends ChangeNotifier {
  ...
  Future<OrderRequest> submit({
    ...
    final request = await _service.submit(
      ...
      title: composeOrderRequestTitle(
        category,
        itemName: composeOrderRequestItemName(
          propertyRequirement: propertyRequirement,
          vehicleRequirement: vehicleRequirement,
          machineryRequirement: machineryRequirement,
          generalRequirement: generalRequirement,
  ...
  /// Orders this agent has claimed and is still responsible for — the ones
  /// that belong under Property Management -> My claims (and its dashboard
  /// counter). Excludes finished ones ([OrderRequestStatus.closed]), the
  /// same way a sell request drops out of "My claims" once it's listed.
  /// A reported ([OrderRequestStatus.disputed]) order stays until Admin
  /// reposts it, so it doesn't silently vanish from the agent's list.
  List<OrderRequest> claimedActiveBy(String agentId) => _requests
      .where((r) =>
          r.assignedAgentId == agentId &&
          (r.status == OrderRequestStatus.agentConfirmed || r.status == OrderRequestStatus.disputed))
      .toList();
