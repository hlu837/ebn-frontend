...
class SellRequestController extends ChangeNotifier {
  ...
  /// Set when the most recent [fetchAgentView] had at least one of its five
  /// underlying calls fail. Previously a single failing call (one flaky
  /// network request, a 401, etc.) made `Future.wait` reject as a whole —
  /// the other four already-succeeded lists were thrown away too, so the
  /// Agent's "Available"/"Claimed" counters silently sat at 0 with no
  /// error shown, looking like claimed/available requests had vanished
  /// even though the data was fine server-side. Screens can read this to
  /// show a retry affordance instead of a bare "nothing here" empty state.
  String? lastError;
  ...
  /// claim pool first.
  Future<void> adminApproveSubmission(String id, {required String token}) async {
  ...
  ///
  /// Each of the five underlying calls is fetched and applied
  /// independently now — previously they were wrapped in one `Future.wait`,
  /// so a single failing call (a flaky request, an expired session, etc.)
  /// made the whole batch reject and discarded the other four lists that
  /// had *already* succeeded. That's what made the "Available"/"Claimed"
  /// counters go blank even when the agent really did have claimed or
  /// available requests: the fetch silently threw before any of it reached
  /// `_requests`. Now a failure in one part just leaves that part stale
  /// (and sets [lastError]) instead of wiping out everything else.
  Future<void> fetchAgentView(String agentId) async {
    isLoading = true;
    notifyListeners();
    final calls = <Future<List<SellRequest>>>[
  ...
  /// the claiming Agent.
  Future<Asset> adminApproveReport(String id, {required String token}) async {
    final r = _requests.firstWhere(
      (r) => r.id == id,
      orElse: () => throw StateError('Sell request $id is not loaded'),
    );
