...
class EbnDemoApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      ...
      // Consumer, not a plain build-time read: `LocaleController` and
      // `ThemeController` are registered on the MultiProvider just above,
      // so we need a context *below* them to watch them, and we need
      // `MaterialApp` to actually rebuild (with a new `Localizations`
      // subtree, or a new theme) whenever someone picks a language or a
      // theme — a one-off read at startup would leave the app pinned to
      // whatever was resolved the first time.
      child: Consumer2<LocaleController, ThemeController>(
        builder: (context, localeController, themeController, _) {
          return MaterialApp(
            ...
            // `null` here (the controller's initial value) makes
            // `localeResolutionCallback` below run against the device's
            // own language; once a user picks one explicitly, this is
            // the pinned `Locale` and the callback is bypassed.
