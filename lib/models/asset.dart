...
/// Mirrors the `asset_status` enum in the DB. `reserved` and `rented`
/// were added by 068_rental_agreements.sql once a rent_now request
/// reaches document review / payment — before that migration this enum
/// only had draft/active/underInspection/sold/archived, which silently
/// mapped anything else back to `active` in [fromApi] below.
enum AssetStatus { draft, active, underInspection, sold, archived, reserved, rented }
...
class Asset {
  ...
  /// Whether this listing is a rental rather than a sale — read from the
  /// generic `attributes` JSONB blob (`listing_type: "rent"`) since the
  /// `assets` table itself has no dedicated column for it yet. Defaults
  /// to false (treated as "for sale") when the backend hasn't sent one.
  bool get isForRent => '${attributes['listing_type']}'.toLowerCase() == 'rent';
  ...
  factory Asset.fromJson(Map<String, dynamic> json) {
    return Asset(
      ...
      imageUrls: (json['image_urls'] as List?)?.whereType<String>().toList() ??
          const [],
  ...
  List<String> get galleryUrls => imageUrls.isNotEmpty
      ? imageUrls
      : (imageUrl != null ? [imageUrl!] : const []);
