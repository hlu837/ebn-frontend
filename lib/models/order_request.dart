...
class PropertyRequirement {
  ...
  /// Short name for exactly what they're after, e.g. "3BR villa in Bole" —
  /// shown as the headline on Order List cards instead of the generic
  /// "Looking for House" when filled in.
  String itemName;
  ...
  String toDescriptionText() {
    ...
    if (itemName.trim().isNotEmpty) buffer.writeln('Looking for: ${itemName.trim()}');
...
class GeneralRequirement {
  /// Short name for exactly what they're after, e.g. "Samsung A4" —
  /// shown as the headline on Order List cards instead of the generic
  /// "Looking for Shop" when filled in.
  String itemName;
  ...
  String toDescriptionText() {
    ...
    buffer.writeln('• ${itemName.trim().isNotEmpty ? itemName.trim() : 'Not specified'}');
    if (description.trim().isNotEmpty) buffer.writeln('• Details: $description');
    buffer.writeln();
...
/// Renders the headline shown on Order List cards / details sheet — the
/// buyer's own short item name (e.g. "Toyota Vitz", "Samsung A4") when they
/// gave one, falling back to the generic "Looking for {category}" when they
/// left it blank. Computed once at submit time and sent to the backend as
/// plain text, same as before.
String composeOrderRequestTitle(AssetCategorySlug category, {String? itemName}) {
  final trimmed = itemName?.trim();
  if (trimmed != null && trimmed.isNotEmpty) return trimmed;
  return 'Looking for ${category.label}';
}

/// Pulls the buyer-entered item name out of whichever requirement variant
/// was filled in — exactly one of the four should be non-null, matching
/// [composeOrderRequestDescription]/[composeOrderRequestBudgetSummary].
String? composeOrderRequestItemName({
  PropertyRequirement? propertyRequirement,
  VehicleRequirement? vehicleRequirement,
  MachineryRequirement? machineryRequirement,
  GeneralRequirement? generalRequirement,
}) {
  final raw = propertyRequirement?.itemName ??
      vehicleRequirement?.itemName ??
      machineryRequirement?.itemName ??
      generalRequirement?.itemName;
