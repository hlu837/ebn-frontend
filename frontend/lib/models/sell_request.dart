...
extension SellRequestStatusX on SellRequestStatus {
  ...
  String get visitorDescription => switch (this) {
        ...
        SellRequestStatus.submissionRejected =>
          'This submission wasn\'t approved. See the note below.',
        SellRequestStatus.broadcasting =>
          'Approved! We\'ve alerted brokers near you — waiting for one to pick it up.',
        SellRequestStatus.openToBrokers =>
          'Approved! Waiting for a broker to pick it up for inspection.',
        SellRequestStatus.claimed =>
          'A broker has been assigned and will visit to inspect the property.',
        SellRequestStatus.reportPendingApproval =>
          'The broker submitted their inspection report — pending final approval.',
        SellRequestStatus.reportRejected =>
          'The inspection report needs changes before it can go live.',
...
class ReportMediaItem {
  ...
  final bool isVideo;
  final String?
      filePath; // Optional: actual file path when using real image picker
  final Uint8List? imageBytes; // In-memory image data for Flutter Web previews
  ...
  Map<String, dynamic> toJson() =>
      {'id': id, 'isVideo': isVideo, 'filePath': filePath};
}
...
class SellRequest {
  ...
  factory SellRequest.fromJson(Map<String, dynamic> json) {
    return SellRequest(
      ...
      broadcastAgentIds: (json['broadcastAgentIds'] as List<dynamic>?)
              ?.map((e) => e as String)
              .toList() ??
          [],
      ...
      reportSubmittedAt: json['reportSubmittedAt'] != null
          ? DateTime.parse(json['reportSubmittedAt'] as String)
          : null,
